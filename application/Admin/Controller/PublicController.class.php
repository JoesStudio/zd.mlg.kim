...
class PublicController extends AdminbaseController {
    ...
    public function dologin(){
    	...
    	if(!sp_check_verify_code()){
    		...
    		if(!empty($result) && $result['user_type']==10){
    ...
    function get_areas(){
        $data['data'] = D('Areas')->where(array('parentId'=>I('get.id')))->select();
        $this->ajaxReturn($data);
    }

    public function go_we7($w_c=null,$w_a=null,$w_m=null,$w_do=null){
        $action = '';
        if(isset($w_c)) $action .= '&c='.$w_c;
        if(isset($w_a)) $action .= '&a='.$w_a;
        if(isset($w_m)) $action .= '&m='.$w_m;
        if(isset($w_do)) $action .= '&do='.$w_do;
        redirect('/we7/web/index.php?go_we7=1'.$action);
    }
