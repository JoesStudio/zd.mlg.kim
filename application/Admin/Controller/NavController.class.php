...
class NavController extends AdminbaseController {
    ...
    public function cat2nav(){
        $catModel = M("TextileFabricCats");
        $P0=$catModel->where(array("pid"=>0))->select();
        foreach ($P0 as $r0) {
            echo "--".$r0["title"]."<br>";
            $pid0=$this->_addnav(1,0,$r0["title"],"","/Tf/List/index/id/".$r0["id"]);
            
           $P1=$catModel->where(array("pid"=>$r0["id"]))->select();
           foreach ($P1 as $r1) {
               echo "-- --".$r1["title"]."<br>";
               $pid1=$this->_addnav(1,$pid0,$r1["title"],"","/Tf/List/index/id/".$r1["id"]);
               
              $P2=$catModel->where(array("pid"=>$r1["id"]))->select();
              foreach ($P2 as $r2) {
                  echo "-- -- --".$r2["title"]."<br>";
                  $pid2=$this->_addnav(1,$pid1,$r2["title"],"","/Tf/List/index/id/".$r2["id"]);
              }
           } 
        }
    }
    private function _addnav($cid,$parentid,$label,$target,$href){
            $data["cid"]=$cid;
