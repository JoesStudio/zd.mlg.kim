...
class UserController extends AdminbaseController{
    ...
    public function ban(){
        $id = I('get.id',0,'intval');
        if (!empty($id)) {
            $result = $this->users_model->where(array("id"=>$id,"user_type"=>10))->setField('user_status','0');
            if ($result!==false) {
    ...
    public function cancelban(){
        $id = I('get.id',0,'intval');
        if (!empty($id)) {
            $result = $this->users_model->where(array("id"=>$id,"user_type"=>10))->setField('user_status','1');
            if ($result!==false) {
    ...
    function search(){
        if($_GET['term']){
            $where['status'] = 1;
            $where['user_type'] = isset($_GET['type']) ? $_GET['type']:array('between', '20,29');
            $where['user_login'] = array('LIKE','%'.$_GET['term'].'%');

            $data = $this->users_model->where($where)->select();
