...
use Common\Model\ColorcardModel;
...
class ColorcardLogic extends ColorcardModel
{
     ...
     * @return array
     */
    function getPaged($where=array(), $pageSize=20){
        $where['card_trash'] = 0;
        $data = $this->cards($where, $pageSize, '', 'Admin');
    ...
    function formatRequest(){
        ...
        foreach($post['pages'] as $k1=>$page){
            foreach($page['items'] as $k2=>$item){
                if($item['tf_id'] > 0){
                    $tf = $tf_model
                        ->field('name,code,img,spec,width,weight,material,component,function,purpose')
                        ->find($item['tf_id']);
