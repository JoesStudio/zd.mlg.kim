...
//                'rootPath' => C("UPLOADPATH"),
        ...
        if ($info) {
            ...
            //记录到面料商的媒体库
            if (isset($_SESSION['user']['member']['id'])) {
                $meta['info'] = $first;
                $data = array(
                    'uid'   => $_SESSION['user']['member']['id'],
                    'utype' => 2,
                    'key'   => $first['key'],
                    'filename'  => $oriName,
                    'filesize'  => $first['size'],
                    'filepath'  => $first['savepath'].$first['savename'],
                    'uploadtime'=> date('Y-m-d H:i:s'),
                    'status'=> 1,
                    'meta'  => json_encode($meta, 256),
                    'suffix'=> $first['ext'],
                );
                D('Asset')->saveAsset($data);
            }
    ...
    private function _get_listimage(){
        ...
        /*if (isset($_SESSION['user']['member'])){
            $path .= 'supplier/'.$_SESSION['user']['member']['id'].'/';
        }*/
        ...
        //echo $path;
        if (!empty(sp_get_current_admin_id())) {
            $files = $this->getfiles($path, $allowFiles);
        } elseif (isset($_SESSION['user']['member'])) {
            $files = $this->getAssets($_SESSION['user']['member']['id'], 2);
        } else {
            $files = $this->getAssets(sp_get_current_userid(), 1);
        }
        if (!count($files)) {
            return json_encode(array(
                "state" => "no match file",
    ...
    private function _get_listimage2(){
        $allowFiles = array( ".gif" , ".png" , ".jpg" , ".jpeg" , ".bmp" ); //文件允许格式
        $listSize = 3000; //文件大小限制，单位KB
        $path = './'. C("UPLOADPATH"); //图片路径
    ...
    {
        $list = D('Asset')->getAssetsNoPaged(array(
            'uid'   => $uid,
            'utype' => $utype,
            'status'=> 1,
        ));
