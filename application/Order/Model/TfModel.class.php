...
class TfModel extends CommonModel
{
    ...
    function getTf($id, $fields="supplier,cat,sku"){
        ...
        if($tf){
            ...
            $tf['tf_code'] = $tf['vend_id'].$tf['cat']['code'].$tf['name_code'].$tf['code'];
            //todo
     ...
     * @return array
     */
    function fabrics($tag='', $pageSize=0, $pagetpl='', $tplName='default'){
        $where = array();
        if(is_array($tag)){
        ...
        $this->alias('tf')->field($field)->join($join)->where($where)->order($order);
        if(empty($pageSize)){
            if (isset($tag['limit'])) {
                $this->limit($limit);
            }
        }else{
            $page = $this->initPager($data['total_condition'], intval($pageSize), $pagetpl, $tplName);
