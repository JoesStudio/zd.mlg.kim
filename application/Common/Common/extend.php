...
//$slice =subWords($str,&$length,$charset);
$slice =my_substr($str,$start,$length);    
...
function subWords($str,&$len,$code){
    $newStr = preg_replace('/[^\x{4e00}-\x{9fa5}]/u', '', mb_substr($str,0,$len,$code));
    $ii = strlen("中");                                    
    $iWord =  mb_strlen($newStr,$code);                        
    $len += ($ii-1)*$iWord;
...
//gbk可以用：
function my_substr2($str, $start, $len)
{
    $tmpstr = "";
    $strlen = $start + $len;
    for($i = 0; $i < $strlen; $i++)
...
function mb_sub($str,$start=0,$len,$encode='utf8'){
        if($encode!='utf8'){
