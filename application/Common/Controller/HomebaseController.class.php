...
class HomebaseController extends AppframeController {
    ...
    protected $site_basic;
    
    public function __construct() {
        $this->set_action_success_error_tpl();
        parent::__construct();

        if (isset($_COOKIE['session_mlg'])) {
            session_id($_COOKIE['session_mlg']);
        }
    }
    ...
    function _initialize() {
        ...
        //条件：手机&&非微信注册
        if(is_weixin() && !isset($_REQUEST['wx_register'])){
            //$_GET['wx_no'] = uniqid();
            if(!sp_get_current_user()){
                $Wx = new \Wx\Common\Wechat();
                if(isset($_GET['code'])){
                    list($err, $user_info) = $Wx->api->get_userinfo_by_authorize('snsapi_userinfo');
                    if ($user_info !== null) {
                        $user = D('Users')->getUserByOpenId($user_info->openid);
                        if($user){
                            session('user',$user);
                            ...
                            //自动注册账号
                            $user = $Wx->regByOpenId($user_info->openid);
                            if($user){
                                session('user',$user);
                                setcookie("uid", sp_get_current_userid(), time() + 3600 * 24, "/", ".mlg.kim");
                                setcookie("session_mlg", session_id(), time() + 3600 * 24, "/", ".mlg.kim");
                                ...
                                $param = array(
                                    ...
                                    'avatar'        => (string)$user_info->headimgurl,
                                );
                                session('login_http_referer',curPageURL());
                                session('wxbind_data', $param);
                                session('short_url',null);
                                $this->redirect('Wx/Login/bind',array('wx_register'=>1));
                            }
                        }
                    } else {
                        exit($err->errcode.':'.$err->errmsg);
                        //$this->error($err[0]->errmsg);
                    }
                }else{
                    $redirect_uri = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/'.ACTION_NAME,$_GET,false,true);
                    $url = $Wx->api->get_authorize_url('snsapi_userinfo',$redirect_uri);
                    redirect($url);
                }
            }
        }
        ...
        $this->site_basic = get_basic_options();
        $this->assign('site_basic',$this->site_basic);

        if(!sp_is_mobile()){
            $footer_news = ApiService::postsNotPaged('limit:3;');
            $this->assign('footer_news', $footer_news);

            if(sp_is_user_login()){
                $unread_msg_num = D('Notify/Msg')->getUnreadNum(sp_get_current_userid(), 1);
                $this->assign('unread_msg_num',$unread_msg_num);
                $unread_msgs = D('Notify/Msg')->getUnread(sp_get_current_userid(), 1);
                $this->assign('unread_msgs',$unread_msgs);

                if (!empty($_SESSION['user']['member'])) {
                    $member_id = $_SESSION['user']['member']['id'];
                    $unread_biz_msg_num = D('Notify/Msg')->getUnreadNum($member_id, 2);
                    $this->assign('unread_biz_msg_num', $unread_biz_msg_num);
                    $unread_biz_msgs = D('Notify/Msg')->getUnread($member_id, 2);
                    $this->assign('unread_biz_msgs', $unread_biz_msgs);
                }
            }
        }
    ...
    protected function check_login(){
        $session_user=session('user');
        if(empty($session_user)){
            //$redirect = leuu('user/login/index',array('redirect'=>base64_encode($_SERVER['HTTP_REFERER'])));
            $redirect = leuu('portal/index/index');
            if(IS_AJAX){
