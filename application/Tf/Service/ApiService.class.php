...
class ApiService {
    ...
    public static function textile_fabrics($tag,$where=array(),$pagesize=0,$pagetpl=''){
        $where=is_array($where)?$where:array();
        $tag=sp_param_lable($tag);    //分解tag参数
    ...
    public static function postsNotPaged($tag,$where=array()){
        $content=self::textile_fabrics($tag,$where);
    ...
    public static function postsPaged($tag,$pagesize=20,$pagetpl=''){
        return self::textile_fabrics($tag,array(),$pagesize,$pagetpl);
    }
    ...
    public static function postsPagedByTermId($term_id,$tag,$pagesize=20,$pagetpl=''){
        ...
        $content=self::textile_fabrics($tag,$where,$pagesize,$pagetpl);
        
        return $content;
    }

    public static function tfPagedByKeyword($keyword,$tag,$pagesize=20,$pagetpl=''){
        $where=array();
        $where['name'] = array('like',"%$keyword%");
        $where['code'] = array('like',"%$keyword%");
        $where['spec'] = array('like',"%$keyword%");
        $where['material'] = array('like',"%$keyword%");
        $where['component'] = array('like',"%$keyword%");
    ...
    public static function tfPagedByUrl($url,$tag,$pagesize=20,$pagetpl=''){
        $cih = new \ImageHash();
        $aHash = $cih::hashImageFile($url);   
       
        $where= 'get_similarity(img_hash,"'.$aHash.'")>0';
        
        $tag=sp_param_lable($tag);    //分解tag参数
        
        $field = '*,get_similarity(img_hash,"'.$aHash.'") as similarity';
        $limit = !empty($tag['limit']) ? $tag['limit'] : '0,10';
        $order = !empty($tag['order']) ? $tag['order'] : 'similarity DESC';
